  ...
  return new Promise(async (resolve,reject)=>{
    try{
      ...
      global.downloadProgress[downloadId]={
        status:'starting', progress:0, downloaded:0, total:0, startTime:Date.now()
      };

      let selectedFormat;
      if(format==='mp3'||format==='m4a'){
        selectedFormat = info.formats.filter(f=>!f.hasVideo&&f.hasAudio)
      ...
      if(format==='mp3'||format==='m4a'){
        ...
        ffmpeg(videoStream).toFormat(format)
